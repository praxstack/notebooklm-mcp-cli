...
def pipeline_run(
    ...
    try:
        ...
        console.print(f"[dim]{result['succeeded']}/{result['total_steps']} steps succeeded "
                      f"({result['total_duration_ms']}ms)[/dim]\n")

        for step in result["steps"]:
            if step["success"]:
                console.print(f"  [green]\u2713[/green] Step {step['step']}: {step['action']} ({step['duration_ms']}ms)")
            else:
                console.print(f"  [red]\u2717[/red] Step {step['step']}: {step['action']} — {step['error']}")
...
def pipeline_list() -> None:
    ...
    for p in pipelines:
        source_badge = "[green]builtin[/green]" if p["source"] == "builtin" else "[yellow]user[/yellow]"
...
def pipeline_create(
    ...
    file: str | None = typer.Option(None, "--file", "-f", help="YAML file with pipeline definition"),
) -> None:
    ...
    try:
        ...
        console.print(f"[green]\u2713[/green] Pipeline '{result['name']}' created ({result['steps_count']} steps)")
    except ServiceError as e:
        console.print(f"[red]Error:[/red] {e.user_message}")
        raise typer.Exit(1)
